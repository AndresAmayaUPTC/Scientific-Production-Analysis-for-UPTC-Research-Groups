...
# Este modulo quedo como BIBLIOTECA DE PARSERS del GrupLac.
#
# La ingesta en uso vive en el paquete `ingesta/`, que reutiliza estas
# funciones de extraccion pero escribe con versionado: nunca borra los datos
# vigentes hasta que la corrida nueva pasa los controles de calidad.
# Ver ingesta/pipeline.py y ingesta/scraper.py.
#
# Aqui ya no se abre ninguna conexion a la base ni se guardan credenciales.
...
def obtener_y_procesar_datos():
    """OBSOLETA. Usa el paquete `ingesta`.

    La version original borraba las colecciones con delete_many() ANTES de
    descargar nada, de modo que cualquier fallo de red dejaba la base vacia
    y la aplicacion sin datos. Ademas construia la lista de miembros pero
    nunca llegaba a insertarla.

    El reemplazo hace lo mismo de forma segura:
        python -m ingesta.run scrapear
    """
    raise NotImplementedError(
        "Esta funcion se retiro por destructiva. Usa: python -m ingesta.run scrapear "
        "(o el endpoint POST /admin/ingesta/ejecutar)."
    )


def actualizar_base_datos():
    """OBSOLETA. Equivalente seguro: `python -m ingesta.run scrapear`."""
    raise NotImplementedError(
        "Esta funcion se retiro por destructiva. Usa: python -m ingesta.run scrapear"
    )
...
if __name__ == "__main__":
    print(__doc__ or "")
    print("Este archivo es una biblioteca de parsers.")
    print("Para actualizar la base de datos ejecuta:  python -m ingesta.run scrapear")
